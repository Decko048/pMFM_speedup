...
def CBIG_mfm_multi_simulation(parameter,
    ...
    for i in range(n_set):
        ...
        I_I_ave[:, i], infodict, ier, mseg = fsolve(I_I_fixed_pt,
                                                    I_I_ave_one_set,
                                                    args=w_EI_one_set,
                                                    full_output=True)
    I_I_ave = torch.from_numpy(I_I_ave).type(torch.DoubleTensor).cuda()
    S_I_ave = tau_I * (a_I * I_I_ave -
                       b_I) / (1 - torch.exp(-d_I * (a_I * I_I_ave - b_I)))

    # calculate J_I
    J_I = torch.div(
        W_E * I0 + w_EE * J_NMDA * S_E_ave + G * J_NMDA *
        torch.sum(sc_mat, 1).view(-1, 1).repeat(1, n_set) * S_E_ave - I_E_ave,
        S_I_ave)
    ...
    for i in range(preheat):
        ...
        if is_memory_insufficient == 0:
            noise_level = d_w[:, :,
                              i].repeat(1, 1,
                                        n_num).contiguous().view(-1, n_nodes)
            S_E = S_E + dS_E * d_t + w_coef * torch.transpose(
                noise_level, 0, 1
            )  # training time step size 6ms; validation + testing time step size 0.5ms;
            S_I = S_I + dS_I * d_t + w_coef * torch.transpose(
                noise_level, 0, 1)
        else:
            S_E = S_E + dS_E * d_t + w_coef * torch.randn(
                n_nodes, n_dup).repeat(1, n_num) * math.sqrt(d_t)
            S_I = S_I + dS_I * d_t + w_coef * torch.randn(
                n_nodes, n_dup).repeat(1, n_num) * math.sqrt(d_t)
    ...
    for i in range(n_samples):
        ...
        y_bold_temp = 100 / p_costant * v_0 * (k_1 * (1 - q_t) + k_2 *
                                               (1 - q_t / v_t) + k_3 *
                                               (1 - v_t))
...
def CBIG_mfm_single_simulation(parameter,
    ...
    for i in range(5000):
        dS_E, dS_I, r_E = CBIG_mfm_rfMRI_ode(S_E, S_I, J_I, parameter, sc_mat)
        S_E = S_E + dS_E * d_t + w_coef * torch.randn(n_nodes,
                                                      n_set) * math.sqrt(d_t)
...
def CBIG_mfm_rfMRI_ode(S_E, S_I, J_I, parameter, sc_mat):
    ...
    I_E = W_E * I0 + J_NMDA * w_EE * S_E + J_NMDA * G.repeat(
        number_roi, 1) * torch.mm(sc_mat, S_E) - J_I * S_I
...
def CBIG_FCcorrelation_multi_simulation(emp_fc, bold_d, n_dup):
    ...
    for k in range(n_dup):
        ...
        sim_fc_den = sim_fc_den + (sim_fc_vector[k * n_num:(k + 1) * n_num,
                                                 0:1] != 0).float()
...
def CBIG_FCDKSstat_multi_simulation(emp_ks, bold_d, n_dup):
    ...
    for b in range(batch_num):
        ...
        for i in range(0, time_lengh):
            ...
            fc_mat[:, :,
                   i] = torch.transpose(cor_temp.view(sub_num, fc_edgenum), 0,
                                        1)

        for j in range(0, sub_num):
            fcd_temp = torch_corr(torch.transpose(fc_mat[:, j, :], 0, 1))
            fcd_hist_temp = np.histogram(fcd_temp[fcd_mask].cpu().numpy(),
                                         bins=10000,
                                         range=(-1., 1.))
    ...
    if resid_num != 0:
        ...
        for i in range(time_lengh):
            ...
            fc_resid[:, :,
                     i] = torch.transpose(cor_temp.view(resid_num, fc_edgenum),
                                          0, 1)
    ...
    for k in range(n_dup):
        fcd_histcum_temp = fcd_histcum_temp + fcd_histcumM[:, k * n_num:
                                                           (k + 1) * n_num]
...
def CBIG_FCDKSstat_single_simulation(emp_ks, bold_d, n_dup, window_size=83):
    ...
    for t in range(time_dup):
        ...
        fcd_histcum_nn = fcd_hist_temp[:, fcd_hist_temp[-1, :] == emp_ks[-1,
                                                                         0]]
...
def CBIG_combined_cost_train(parameter, n_dup, FCD, SC, FC, countloop):
    ...
    bold_d, S_E_all, S_I_all, r_E_all, J_I = CBIG_mfm_multi_simulation(
        parameter, sc_mat, 14.4, n_dup, countloop, 0.006, 0)

    # Calculating FC correlation cost
    fc_corr_cost, fc_L1_cost, emp_fc_mean = CBIG_FCcorrelation_multi_simulation(
        emp_fc, bold_d, n_dup)
...
def CBIG_combined_cost_validation(parameter, n_dup, FCD, SC, FC):
    ...
    # Calculating simualted BOLD signal using MFM
    bold_d, S_E_all, S_I_all, r_E_all, J_I = CBIG_mfm_multi_simulation(
        parameter, sc_mat, 14.4, n_dup, 1, 0.0005,
        1)  # the first '1' here is just a placeholder

    # Calculating FC correlation cost
    fc_cost, fc_L1_cost, _ = CBIG_FCcorrelation_multi_simulation(
        emp_fc, bold_d, n_dup)
...
def CBIG_combined_cost_test(parameter, n_dup, FCD, SC, FC):
    ...
    # Calculating simualted BOLD signal using MFM
    bold_d, S_E_all, S_I_all, r_E_all, J_I = CBIG_mfm_single_simulation(
        parameter, sc_mat, 14.4)

    # Calculating FC correlation cost
    fc_corr_cost, fc_L1_cost = CBIG_FCcorrelation_single_simulation(
        emp_fc, bold_d, n_dup)
