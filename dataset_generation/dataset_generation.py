...
import time
import torch
import CBIG_pMFM_basic_functions_HCP as fc
import warnings
import sys
import os
...
def get_init(myelin_data, gradient_data, num_myelin_component,
             num_gradient_component, init_para):
    '''
    ...
    concat_matrix = np.vstack(
        (np.ones(n_node), myelin_data.T,
         gradient_data.T)).T  # bias, myelin PC, RSFC gradient PC
    para = np.linalg.inv(
...
def CBIG_mfm_optimization_desikan_main(input_args):
    ...
    gradient_data = fc.csv_matrix_read(path_to_group +
                                       'group_level_RSFC_gradient.csv')
    num_gradient_component = gradient_data.shape[1]
    N = 3 * (num_myelin_component + num_gradient_component +
             1) + 1  # number of parameterized parameters 10
    ...
    search_range[n_node:n_node * 2, :] = [wEI_min,
                                          wEI_max]  # search range for w_EI
    search_range[n_node * 2, :] = [0, 3]  # search range for G
    search_range[n_node * 2 + 1:dim, :] = [0.0005,
                                           0.01]  # search range for sigma
    init_para = rng.uniform(0, 1, dim) * (
        search_range[:, 1] - search_range[:, 0]) + search_range[:, 0]
    start_point_w_EE, template_mat = get_init(myelin_data, gradient_data,
                                              num_myelin_component,
                                              num_gradient_component,
                                              init_para[0:n_node])
    start_point_w_EI, template_mat = get_init(myelin_data, gradient_data,
                                              num_myelin_component,
                                              num_gradient_component,
    ...
    all_parameters = np.zeros(
        (209, maxloop * Lambda
         ))  # numpy array to store all parameters and their associated costs
    ...
    with open(output_path + output_file, 'w') as f:
        while countloop < maxloop:
            iteration_log = open(output_path + 'training_iteration' + '.txt',
                                 'w')
            ...
            all_parameters[4:, countloop * Lambda:(countloop + 1) *
                           Lambda] = input_para
            all_parameters[0, countloop * Lambda:(countloop + 1) *
                           Lambda] = fc_corr_cost
            all_parameters[1, countloop * Lambda:(countloop + 1) *
                           Lambda] = fc_L1_cost
            all_parameters[2, countloop * Lambda:(countloop + 1) *
                           Lambda] = fcd_cost
            all_parameters[3, countloop * Lambda:(countloop + 1) *
                           Lambda] = total_cost
            ...
            pc = (1 - cc) * pc + hsig * np.sqrt(
                cc * (2 - cc) * mueff) * xshow / sigma

            # Adapting covariance matrix C
            artmp = (1 / sigma) * (arx[:, arindex[0:mu]] -
                                   np.tile(xold, [mu, 1]).T)
            ...
            cost_log.write('wEE search range: ' + str(wEE_min) + ', ' +
                           str(wEE_max) + '\n')
            cost_log.write('wEI search range: ' + str(wEI_min) + ', ' +
                           str(wEI_max) + '\n')

            elapsed_time = time.time() - start_time
            print('Elapsed time for this evolution is : ', elapsed_time)
            cost_log.write('Elapsed time for this evolution is : ' +
                           str(elapsed_time) + '\n')
            print('******************************************')
            cost_log.write('******************************************')
            cost_log.write("\n")
...
def generate_parameters_for(split_name, idx_in_split_range):
    ...
    for idx_in_split in idx_in_split_range:
        ...
        input_args.append([
            output_path, split_name + '_' + idx_in_split + '.csv',
            path_to_group, random_seed
        ])
...
def split(r, n_batches):
    ...
    return list((r[i * k + min(i, m):(i + 1) * k + min(i + 1, m)]
