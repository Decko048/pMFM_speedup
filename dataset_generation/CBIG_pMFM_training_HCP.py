...
def CBIG_mfm_optimization_desikan_main(input_args):
    ...
    myelin_data = fc.csv_matrix_read(
        '/home/shaoshi.z/storage/MFM/Alpraz_EI_dataset/TASK/results/HCP/input/myelin.csv'
    )
    ...
    start_point_w_EE, template_mat = get_init(
        myelin_data, gradient_data, num_myelin_component,
        num_gradient_component, init_para[0:n_node])
    start_point_w_EI, template_mat = get_init(
        myelin_data, gradient_data, num_myelin_component,
        num_gradient_component, init_para[n_node:n_node * 2])
    start_point_sigma, template_mat = get_init(
        myelin_data, gradient_data, num_myelin_component,
        num_gradient_component, init_para[n_node * 2 + 1:dim])
    ...
    with open(output_path + output_file, 'a') as f:
        while countloop < maxloop:
            ...
            while j < Lambda:
                arx[:, j] = xmean + sigma * np.dot(
                    B, (D * rng.standard_normal(N)))
                input_para[0:n_node, j] = template_mat @ arx[0:N_p, j]
                input_para[n_node:2 *
                           n_node, j] = template_mat @ arx[N_p:2 * N_p, j]
                input_para[2 * n_node:2 * n_node + 1, j] = arx[2 * N_p, j]
                input_para[2 * n_node +
                           1:dim, j] = template_mat @ arx[2 * N_p + 1:N, j]
            ...
            xmin[N + 3] = np.min(
                total_cost
            )  # one among all children # TODO: output all 100 children processes
...
if __name__ == "__main__":
    ...
    input_args = ([
        output_path, 'training_' + str(random_seed[0]) + '.csv', FCD, SC, FC,
        random_seed[0]
    ], [
        output_path, 'training_' + str(random_seed[1]) + '.csv', FCD, SC, FC,
        random_seed[1]
    ], [
        output_path, 'training_' + str(random_seed[2]) + '.csv', FCD, SC, FC,
        random_seed[2]
    ], [
        output_path, 'training_' + str(random_seed[3]) + '.csv', FCD, SC, FC,
        random_seed[3]
    ], [
        output_path, 'training_' + str(random_seed[4]) + '.csv', FCD, SC, FC,
        random_seed[4]
    ])
    p = Pool(5)
